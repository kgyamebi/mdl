...
test('sales POS completes shop A sale', async ({ page }) => {
  ...
  expect(saleBody.data.saleNumber).toMatch(/^SALE-\d{4}-\d+$/);
...
test('owner can create user with autogenerated credentials', async ({ page }) => {
  await loginAsOwner(page);
  await page.goto('/admin/users');

  const suffix = Date.now().toString().slice(-6);
  await page.getByRole('button', { name: 'New user' }).click();
  await page.getByLabel('First name').fill(`New${suffix}`);
  await page.getByLabel('Last name').fill('Staff');
  await page.getByRole('group', { name: 'Shops they can sell from' }).getByRole('checkbox').first().check();
  await page.getByRole('button', { name: 'Create user' }).click();

  await expect(page.getByRole('heading', { name: 'User created' })).toBeVisible({ timeout: 10000 });
  const credentialsPanel = page.locator('.panel--success');
  await expect(credentialsPanel.getByText(/@mdl\.local$/)).toBeVisible();
  await expect(credentialsPanel.locator('code')).toBeVisible();
});

test('owner can sign out and sign back in', async ({ page }) => {
  await loginAsOwner(page);
...
test('owner can open admin locations page', async ({ page }) => {
  await loginAsOwner(page);
  await page.goto('/admin/locations');
  await expect(page.getByRole('heading', { name: 'Locations & routes' })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Add shop' })).toBeVisible();
});
